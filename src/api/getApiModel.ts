...
import { InterfaceNames, ParaPayloadName, PayloadContent } from './interfaces';

const extraFetchOptionsInterfaceName = 'ExtraFetchOptions';
const extraFetchOptionsParaName = 'extraFetchOptions';

export default (items: CustomPath[], key: string, basePath: string, translateLongToString: boolean = false) => {
    const imports = [`import request from '@/utils/request';`, `import { ${extraFetchOptionsInterfaceName} } from '@/types';`];
    const { content, globalInterfaceNames, hasQuery } = renderContent(items, basePath, translateLongToString);
    if (globalInterfaceNames.length > 0) {
        ...
        // imports.push(`export { ${globalInterfaceNames.join(', ')} };`);
    }
    if (hasQuery) {
        imports.unshift(`import stringify from '@/utils/stringify';`);
    }
    return `${imports.join('\n')}\n\n${content}`;
};

function renderContent(items: CustomPath[], basePath: string, translateLongToString: boolean) {
    const globalInterfaceNames = new Set();
    const content = `
${items
    .map((item) => {
        const { pathKey, url, method, parametersInBody = [], parametersInQuery = [], deprecated, summary } = item;
        ...
        const { interfaces, itemInterfaceNames, responseInterfaceName } = renderMethodInterface(item, interfaceNames, translateLongToString);
        itemInterfaceNames.forEach((itemInterfaceName) => globalInterfaceNames.add(itemInterfaceName));
...
export async function ${pathKey}(${renderArgs(interfaces, interfaceNames.payload)}) {${paramsString}
    return request<${responseInterfaceName}>(\`${fullUrl}${parametersInQuery.length > 0 ? '?${stringify(query)}' : ''}\`, {${
...
`;
    ...
    .join('')}
`;
    return { content, globalInterfaceNames: [...globalInterfaceNames], hasQuery: items.some((item) => (item.parametersInQuery || []).length > 0) };
}
...
function renderPayloadInterface(items: PayloadContent[], interfaceName: string) {
    return `export interface ${interfaceName} extends ${extraFetchOptionsInterfaceName} {\n\t${items
        .map((item) => `${getPayloadInterfaceParaDescription(item)}${item.name}: ${item.typeName}`)
        .join('\n\t')}\n}`;
}
 ...
 */
function renderMethodInterface(item: CustomPath, interfaceNames: InterfaceNames, translateLongToString: boolean) {
    const { parametersInQuery = [], parametersInBody = [], parametersInFormData = [], parametersInPath = [], responses } = item;
    ...
    const queryInterface = renderInterface(parametersInQuery, interfaceNames.query, translateLongToString);
    if (queryInterface) {
        payloadContent.push({
    ...
    const getTypeName = (target: Response) => {
        ...
        if (target.schema.$$ref) {
            ...
            typeName = getType(target.schema.type!, target.schema.items, translateLongToString);
        }
        return typeName;
    };
    ...
    if (parametersInFormData.length > 0) {
        payloadContent.push({
            name: ParaPayloadName.body,
            typeName: 'FormData',
            description: `${renderInterface(parametersInFormData, 'FormContent', translateLongToString)}`,
        });
    }
 ...
 * @param {string} name
 */
function renderInterface(parameters: Parameter[], name: string, translateLongToString: boolean) {
    ...
    const result = getInterface(schema, 1, translateLongToString);
 ...
 * @param {string} type
 */
function getType(type: string, items: Schema = { type: Type.any }, translateLongToString?: boolean): string {
    if (type === Type.array) {
        if (isLong(items) && translateLongToString) {
            return `string[]`;
        }
        if (isEnum(items)) {
