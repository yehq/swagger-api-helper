...
var generate = function (options) {
    var urls = options.urls, outputPath = options.outputPath, fetchOptions = options.fetchOptions;
    var targetUrls = urls.map(function (url, index) {
        if (typeof url === 'string') {
            return [url, "swaggerApi" + index, fetchOptions];
        }
        return (url[2] ? url : [url[0], url[1], fetchOptions]);
    });
    var genApis = targetUrls.map(function (_a, index) {
        var _b = __read(_a, 3), url = _b[0], dirname = _b[1], currentFetchOptions = _b[2];
        return new Promise(function (resolve, reject) {
            var successMessages = [];
            var errorMessages = [];
            return utils_1.fetchSwaggerJson(url, currentFetchOptions)
                .then(function (data) {
                var count = 0;
