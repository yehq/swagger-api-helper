...
exports.default = (function (items, key, basePath, translateLongToString) {
    if (translateLongToString === void 0) { translateLongToString = false; }
    var imports = ["import request from '@/utils/request';", "import { " + extraFetchOptionsInterfaceName + " } from '@/types';"];
    var _a = renderContent(items, basePath, translateLongToString), content = _a.content, globalInterfaceNames = _a.globalInterfaceNames, hasQuery = _a.hasQuery;
...
function renderContent(items, basePath, translateLongToString) {
    ...
    var content = "\n" + items
        ...
        var _c = renderMethodInterface(item, interfaceNames, translateLongToString), interfaces = _c.interfaces, itemInterfaceNames = _c.itemInterfaceNames, responseInterfaceName = _c.responseInterfaceName;
        itemInterfaceNames.forEach(function (itemInterfaceName) { return globalInterfaceNames.add(itemInterfaceName); });
        ...
        .join('') + "\n";
    return { content: content, globalInterfaceNames: __spread(globalInterfaceNames), hasQuery: items.some(function (item) { return (item.parametersInQuery || []).length > 0; }) };
}
/**
...
function renderPayloadInterface(items, interfaceName) {
    return "export interface " + interfaceName + " extends " + extraFetchOptionsInterfaceName + " {\n\t" + items
        .map(function (item) { return "" + getPayloadInterfaceParaDescription(item) + item.name + ": " + item.typeName; })
        .join('\n\t') + "\n}";
}
// 生成 payload 中的 description
function getPayloadInterfaceParaDescription(item) {
 ...
 */
function renderMethodInterface(item, interfaceNames, translateLongToString) {
    ...
    var queryInterface = renderInterface(parametersInQuery, interfaceNames.query, translateLongToString);
    if (queryInterface) {
        payloadContent.push({
            name: "query" /* query */,
 ...
 * @param {string} name
 */
function renderInterface(parameters, name, translateLongToString) {
    ...
    var result = getInterface_1.default(schema, 1, translateLongToString);
 ...
 * @param {string} type
 */
function getType(type, items, translateLongToString) {
    if (items === void 0) { items = { type: "any" /* any */ }; }
    if (type === "array" /* array */) {
        if (isTypes_1.isLong(items) && translateLongToString) {
            return "string[]";
        }
